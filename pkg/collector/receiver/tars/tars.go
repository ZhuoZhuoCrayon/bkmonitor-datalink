...
func Ready(config receiver.ComponentConfig) {
	...
	statImpl := NewStatImpl()
	statDispatch := new(statf.StatF)
	receiver.RegisterRecvTarsRoute("StatObj", "tarsstat", statImpl, statDispatch)
	propertyImpl := NewPropertyImpl()
	propertyDispatch := new(propertyf.PropertyF)
	receiver.RegisterRecvTarsRoute("PropertyObj", "tarsproperty", propertyImpl, propertyDispatch)
}
...
func (imp *StatImpl) ReportMicMsg(tarsCtx context.Context, stats map[statf.StatMicMsgHead]statf.StatMicMsgBody, bFromClient bool) (int32, error) {
	...
	if len(token) == 0 {
		// 尝试从 ctx 中取 token
		token = define.TokenFromTarsCtx(tarsCtx)
	}

	ipFromContext := utils.GetTarsIpFromContext(tarsCtx)
	...
	r := &define.Record{
		...
		Data: &define.TarsData{
			...
			Data:      &define.TarsStatData{Stats: stats},
		},
