...
func TestPropertyImpl(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("failed to invoke ReportPropMsg, err=%v", err)
		}
		...
		for head, body := range pd.Props {
			if hj, err := json.Marshal(head); err == nil {
				exceptHj := `{"moduleName":"TestApp.HelloGo","ip":"127.0.0.1","propertyName":"Add","setName":"","setArea":"","setID":"","sContainer":"","iPropertyVer":2}`
				assert.JSONEq(t, exceptHj, string(hj))
			} else {
				t.Fatalf("failed to Marshal prop head, err=%v", err)
			}
			if bj, err := json.Marshal(body); err == nil {
				exceptBj := `{"vInfo":[{"policy":"Sum","value":"440"},{"policy":"Avg","value":"73.333"},{"policy":"Max","value":"94"},{"policy":"Min","value":"33"},{"policy":"Count","value":"6"},{"policy":"Distr","value":"0|0,50|1,100|5"}]}`
				assert.JSONEq(t, exceptBj, string(bj))
			} else {
				t.Fatalf("failed to Marshal prop body, err=%v", err)
			}
		}
	})
}
...
func TestStatImpl(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		for head, body := range pd.Stats {
			if hj, err := json.Marshal(head); err == nil {
				exceptHj := `{"masterName":"stat_from_server","slaveName":"TestApp.HelloGo","interfaceName":"Add","masterIp":"127.0.0.1","slaveIp":"127.0.0.1","slavePort":0,"returnValue":0,"slaveSetName":"","slaveSetArea":"","slaveSetID":"","tarsVersion":"1.4.5"}`
				assert.JSONEq(t, exceptHj, string(hj))
			} else {
				t.Fatalf("failed to Marshal prop head, err=%v", err)
			}
			if bj, err := json.Marshal(body); err == nil {
				exceptBj := `{"count":6,"timeoutCount":0,"execCount":0,"intervalCount":{"100":0,"200":2,"500":4},"totalRspTime":1343,"maxRspTime":284,"minRspTime":159}`
				assert.JSONEq(t, exceptBj, string(bj))
			} else {
				t.Fatalf("failed to Marshal prop body, err=%v", err)
			}
